...
# iptables -t mangle -A GOST -p udp -d 10.0.0.0/16 -j RETURN
iptables -t mangle -A GOST -p udp -d 100.64.0.0/24 -j RETURN
iptables -t mangle -A GOST -p udp -d 169.254.0.0/16 -j RETURN
iptables -t mangle -A GOST -p udp -d 10.0.0.2/32 -j RETURN
...
iptables -t mangle -A GOST -p udp -j TPROXY -d 10.0.0.1/32 --tproxy-mark 0x1/0x1 --on-ip 127.0.0.1 --on-port 12345
iptables -t mangle -A GOST -p udp -j RETURN 
iptables -t mangle -A PREROUTING -p udp -j GOST

iptables -t mangle -N GOST_LOCAL

# 排除DNS服务器的流量
iptables -t mangle -A GOST_LOCAL -p udp -d 8.8.8.8 -j RETURN
iptables -t mangle -A GOST_LOCAL -p udp -d 8.8.4.4 -j RETURN
iptables -t mangle -A GOST_LOCAL -p udp -d 1.1.1.1 -j RETURN
iptables -t mangle -A GOST_LOCAL -p udp -d 9.9.9.9 -j RETURN
iptables -t mangle -A GOST_LOCAL -p udp -d 114.114.114.114 -j RETURN
iptables -t mangle -A GOST_LOCAL -p udp -d 100.64.0.0/24 -j RETURN
...
iptables -t mangle -A GOST_LOCAL -p udp -d 10.0.0.2/32 -j RETURN
# iptables -t mangle -A GOST_LOCAL -p udp -d 10.0.0.0/16 -j RETURN
iptables -t mangle -A GOST_LOCAL -p udp -d 169.254.0.0/16 -j RETURN
# iptables -t mangle -A GOST_LOCAL -p udp -d 10.0.7.1/32 -j RETURN
# iptables -t mangle -A GOST_LOCAL -p udp -d 10.0.4.2/32 -j RETURN
# iptables -t mangle -A GOST_LOCAL -p udp -d 119.45.114.162/32 -j RETURN
...
iptables -t mangle -A GOST_LOCAL -p udp -d 10.0.0.1/32 -j MARK --set-mark 1
